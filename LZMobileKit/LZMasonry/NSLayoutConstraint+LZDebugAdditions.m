//
//  NSLayoutConstraint+LZDebugAdditions.m
//  Masonry
//
...
#import "NSLayoutConstraint+LZDebugAdditions.h"
#import "LZConstraint.h"
#import "LZLayoutConstraint.h"

@implementation NSLayoutConstraint (LZDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(LZLayoutPriorityDefaultHigh)      : @"high",
            @(LZLayoutPriorityDefaultLow)       : @"low",
            @(LZLayoutPriorityDefaultMedium)    : @"medium",
            @(LZLayoutPriorityRequired)         : @"required",
            @(LZLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(LZLayoutPriorityDefaultHigh)                 : @"high",
            @(LZLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(LZLayoutPriorityDefaultMedium)               : @"medium",
            @(LZLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(LZLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(LZLayoutPriorityDefaultLow)                  : @"low",
            @(LZLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(LZLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(lz_key)] && [obj lz_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj lz_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != LZLayoutPriorityRequired) {
