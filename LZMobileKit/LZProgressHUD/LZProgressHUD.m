...
#define LZMainThreadAssert() NSAssert([NSThread isMainThread], @"LZProgressHUD needs to be accessed on the main thread.");

CGFloat const LZProgressMaxOffset = 1000000.f;

static const CGFloat LZDefaultPadding = 4.f;
static const CGFloat LZDefaultLabelFontSize = 16.f;
static const CGFloat LZDefaultDetailsLabelFontSize = 12.f;


@interface LZProgressHUD () {
    // Deprecated
...
@interface LZProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LZProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LZProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LZProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LZProgressHUD *hud = (LZProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LZProgressHUDAnimationFade;
    _mode = LZProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(LZProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LZProgressHUDAnimationZoom) {
        type = animatingIn ? LZProgressHUDAnimationZoomIn : LZProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LZProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LZProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LZProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LZProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    LZProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LZProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LZBackgroundView *backgroundView = [[LZBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LZProgressHUDBackgroundStyleSolidColor;
    ...
    LZBackgroundView *bezelView = [LZBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:LZDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:LZDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [LZProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:LZDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    LZProgressHUDMode mode = self.mode;
    if (mode == LZProgressHUDModeIndeterminate) {
    ...
    else if (mode == LZProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[LZBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LZProgressHUDModeDeterminate || mode == LZProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[LZRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == LZProgressHUDModeAnnularDeterminate) {
            [(LZRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LZProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LZProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LZProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LZProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[LZRoundProgressView class]]) {
        LZRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LZRoundProgressView appearanceWhenContainedIn:[LZProgressHUD class], nil];
#else
        appearance = [LZRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LZProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((LZRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((LZRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[LZBarProgressView class]]) {
        LZBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LZBarProgressView appearanceWhenContainedIn:[LZProgressHUD class], nil];
#else
        appearance = [LZBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LZProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LZBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LZBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LZBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? LZDefaultPadding : 0.f;
...
- (void)setMode:(LZProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LZProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation LZBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = LZProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = LZProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(LZProgressHUDBackgroundStyle)style {
    if (style == LZProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = LZProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    LZProgressHUDBackgroundStyle style = self.style;
    if (style == LZProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LZProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation LZProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LZProgressHUD allHUDsForView:view];
    for (LZProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LZProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    LZBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == LZProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    LZMainThreadAssert();
    self.backgroundView.style = LZProgressHUDBackgroundStyleSolidColor;
...
@implementation LZProgressHUDRoundedButton
