...
+ (NSMutableDictionary *)lz_propertyDictForKey:(const void *)key
{
...
#pragma mark - --私有方法--
+ (id)lz_propertyKey:(NSString *)propertyName
{
    LZExtensionAssertParamNotNil2(propertyName, nil);
    
    __block id key = nil;
    // 查看有没有需要替换的key
    if ([self respondsToSelector:@selector(lz_replacedKeyFromPropertyName121:)]) {
        key = [self lz_replacedKeyFromPropertyName121:propertyName];
    }
    ...
    // 查看有没有需要替换的key
    if ((!key || [key isEqual:propertyName]) && [self respondsToSelector:@selector(lz_replacedKeyFromPropertyName)]) {
        key = [self lz_replacedKeyFromPropertyName][propertyName];
    }
...
+ (Class)lz_propertyObjectClassInArray:(NSString *)propertyName
{
    __block id clazz = nil;
    if ([self respondsToSelector:@selector(lz_objectClassInArray)]) {
        clazz = [self lz_objectClassInArray][propertyName];
    }
...
+ (void)lz_enumerateProperties:(LZPropertiesEnumeration)enumeration
{
    ...
    NSArray *cachedProperties = [self lz_properties];
...
#pragma mark - 公共方法
+ (NSMutableArray *)lz_properties
{
    NSMutableArray *cachedProperties = [self lz_propertyDictForKey:&LZCachedPropertiesKey][NSStringFromClass(self)];
    if (cachedProperties == nil) {
    
        if (cachedProperties == nil) {
            cachedProperties = [NSMutableArray array];
            
            [self lz_enumerateClasses:^(__unsafe_unretained Class c, BOOL *stop) {
                // 1.获得所有的成员变量
                ...
                for (unsigned int i = 0; i<outCount; i++) {
                    ...
                    property.srcClass = c;
                    [property setOriginKey:[self lz_propertyKey:property.name] forClass:self];
                    [property setObjectClassInArray:[self lz_propertyObjectClassInArray:property.name] forClass:self];
            ...
            [self lz_propertyDictForKey:&LZCachedPropertiesKey][NSStringFromClass(self)] = cachedProperties;
        }
    }
...
+ (id)lz_getNewValueFromObject:(__unsafe_unretained id)object oldValue:(__unsafe_unretained id)oldValue property:(LZProperty *__unsafe_unretained)property{
    // 如果有实现方法
    if ([object respondsToSelector:@selector(lz_newValueFromOldValue:property:)]) {
        return [object lz_newValueFromOldValue:oldValue property:property];
    }
...
{
    [self lz_setupBlockReturnValue:objectClassInArray key:&LZObjectClassInArrayKey];
    
    LZExtensionSemaphoreCreate
    LZExtensionSemaphoreWait
    [[self lz_propertyDictForKey:&LZCachedPropertiesKey] removeAllObjects];
    LZExtensionSemaphoreSignal
}
...
{
    [self lz_setupBlockReturnValue:replacedKeyFromPropertyName key:&LZReplacedKeyFromPropertyNameKey];
