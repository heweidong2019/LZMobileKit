...
{
    LZProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    LZExtensionAssertParamNotNil(property);
    ...
    _type = [LZPropertyType cachedTypeWithCode:code];
}
...
- (id)valueForObject:(id)object
{
    ...
    /** https://github.com/CoderLZLee/LZExtension/issues/545 */
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                LZPropertyKey *propertyKey = [[LZPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                LZPropertyKey *subPropertyKey = [[LZPropertyKey alloc] init];
                subPropertyKey.type = LZPropertyKeyTypeArray;
...
- (void)setPorpertyKeys:(NSArray *)propertyKeys forClass:(Class)c
{
    ...
    LZ_LOCK(self.propertyKeysLock);
    self.propertyKeysDict[key] = propertyKeys;
    LZ_UNLOCK(self.propertyKeysLock);
}
...
- (void)setObjectClassInArray:(Class)objectClass forClass:(Class)c
{
    ...
    LZ_LOCK(self.objectClassInArrayLock);
    self.objectClassInArrayDict[key] = objectClass;
    LZ_UNLOCK(self.objectClassInArrayLock);
}
